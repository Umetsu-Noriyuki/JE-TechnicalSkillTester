...
import { searchExamResults as searchExamResultsImpl } from './server/entry_points/search_exam_results';
import { submitResult as submitResultImpl } from './server/entry_points/submit_result';
import { verifyViewerAccessKey as verifyViewerAccessKeyImpl } from './server/entry_points/verify_viewer_access_key';
...
function verifyViewerAccessKey(logRowNumber: number, accessKey: string): boolean {
  return verifyViewerAccessKeyImpl(logRowNumber, accessKey);
}

function searchExamResults(accessKey: string, filter: ExamResultSearchFilter): ExamResultSummary[] {
  return searchExamResultsImpl(accessKey, filter);
}
